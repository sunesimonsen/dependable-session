...
import { observable, flush, subscribables } from "@dependable/state";
...
describe("saveSession, restoreSession, saveSession, restoreSession", () => {
  let nullObservable,
    falseObservable,
    arrayObservable,
    objectObservable,
    anonymousObservable,
    newAnonymousObservable,
    mainObservable;
  ...
  beforeEach(() => {
    ...
    newAnonymousObservable = observable("Another anonymous observable");
    mainObservable = observable(null, { id: "main" });
    mainObservable({ ...mainObservable(), newAnonymousObservable });

    saveSession();
  });

  it("preserves ids", () => {
    const snapshot = JSON.parse(sessionStorage.getItem("@dependable/session"));
    ...
    expect(snapshot, "to equal", {
      ...
      observables: {
        ...
        $1: { $reference: "$0" },
        $2: "Another anonymous observable",
        main: {
          plainValue: 42,
          ...
          newAnonymousObservable: { $reference: "$2" },
        },
      },
    });
  });
